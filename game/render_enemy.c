...
/*   Updated: 2025/02/20 15:29:05 by ybouanan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int generate_random_move(a_data *box, int *new_x , int *new_y)
{
    ...
    *new_y = box->index_enemy[0];
    ...
    if (box->map[*new_y][*new_x] != 'E' && box->map[*new_y][*new_x] != 'C' && box->map[*new_y][*new_x] != '1')
        return 1;
    else (generate_random_move(box, new_x, new_y));
        return 1;
}
...
int	check_if_enemy_stopped(a_data *box)
{
    ...
    x = box->index_enemy[1];
    y = box->index_enemy[0];
    if ((box->map[y][x + 1] == '1' || box->map[y][x + 1] == 'E' || box->map[y][x + 1] == 'C') &&
        (box->map[y][x - 1] == '1' || box->map[y][x - 1] == 'E' || box->map[y][x - 1] == 'C') &&
        (box->map[y + 1][x] == '1' || box->map[y + 1][x] == 'E' || box->map[y + 1][x] == 'C') &&
        (box->map[y - 1][x] == '1' || box->map[y - 1][x] == 'E' || box->map[y - 1][x] == 'C'))
        return (1);
    return (0);
}
